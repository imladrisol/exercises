...
class User
{
	...
	public function __construct()
	{
		...
		//$this->db = new SQLiteDatabase($this->dbName);
                $this->db = new PDO("mysql:host=localhost;dbname=new; charset=utf8","root","");
	...
	public function validate($userInfo)
	{
		...
		/*$userId = $this->db->singleQuery("SELECT id FROM user 
                ...
                $sql = $this->db->prepare('SELECT id FROM user 
                                            WHERE login=:login AND password = :hash');
                $userId = $sql->execute(array("hash"=>$hash, 'login'=>$login));
	...
	public function refreshSession($ticket)
	{
		...
		/*$this->db->query("UPDATE session SET last_request = $time 
				WHERE sess_id = '$ticketId'");*/
                 ...
                 $sql = $this->db->prepare('UPDATE session SET last_request = :time 
				WHERE sess_id = :ticketId');
                $sql->execute(array('time'=>$time, 'ticketId'=>$ticketId));
                $rowsChanged = $sql->rowCount();
               
                //echo $rowsChanged;die;
		...
		//$this->db->query("DELETE FROM session WHERE last_request < $deadTime");
                $sql = $this->db->prepare('DELETE FROM session WHERE last_request < : deadTime');
                $sql->execute(array("deadTime"=>$deadTime));
	...
	public function getOnlineUsers()
	{
		/*$res = $this->db->query("SELECT name, login 
                ...
                $sql = $this->db->prepare('SELECT name, login 
				FROM user, session
				WHERE user.id = session.user_id');
                $sql->execute();
